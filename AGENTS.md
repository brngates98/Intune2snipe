...
- **Tests:** `pytest` in `tests/`; CI runs tests + `helm lint`, then Docker build/push; on `v*` tags the same workflow continues with Helm package, GitHub Release, and Pages (`.github/workflows/docker-build.yml`). **Gitleaks** runs on pushes/PRs to `main`/`master` via `.github/workflows/gitleaks.yml`.
...
- **Auth:** Azure AD **app registration** with **client credentials** (daemon / no user). The app acquires a token with scope **`https://graph.microsoft.com/.default`** using either MSAL `ConfidentialClientApplication` and **`AZURE_CLIENT_SECRET`**, or **`azure.identity.CertificateCredential`** with PEM material (`AZURE_COMBINED_CERT_KEY` or `AZURE_CERTIFICATE_PEM` + `AZURE_PRIVATE_KEY_PEM`). If both are configured, certificate is tried first with optional fallback to the secret. Required env: `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, plus one valid credential path (see [Configuration](docs/configuration.md) and [Certificate authentication](docs/CERTIFICATE_CONFIG.md)).
