...
- **CI:** **pytest** and **`helm lint`** run on **every** push to any branch, on **pull requests** to `main`/`master`, on **`v*`** tags, and on **workflow_dispatch**. Fork PRs are covered by the `pull_request` trigger; same-repo branches are covered by `push` to `**` (you may see two runs when updating an open PR: `push` + `pull_request`).
- **Images:** **Not** built or pushed on ordinary merges to `main` (so every PR merge does not publish a new GHCR image). Docker images are **built and pushed** when you push a **semver tag** (`v*`) or run **Actions → “CI, Docker, and Release” → Run workflow** (`workflow_dispatch`), which can refresh `:latest`. **PRs** that change application code still **build** the image in CI (without pushing) to validate the `Dockerfile`. **Semver tags** always run the full release pipeline (Docker + Helm + Pages), including when the tagged commit is docs-only.
- **Chart:** Linted on every workflow run that includes the `test` job (`helm lint`). Packaged and published **only** on semver tags in the same workflow as Docker—no manual `helm package` for official artifacts.
